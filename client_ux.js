/* client_ux: print + mobile + draft + tek + purge (consolidated) */
(function(){
  if(document.getElementById("print_css_fix"))return;
...
(function(){
  ...
  function enhanceInput(el){
    ...
    el.addEventListener("focus",function(){
      var v=el.value;
      if(v==="0"||v===""||Number(v)===0){el.value="";}
      else{try{el.select();}catch(e){}}
    });
  ...
  function enhanceAll(){
    ...
    ["cashAmount","cardTotal","cashBalance"].forEach(function(id){enhanceInput(document.getElementById(id));});
  }
  function wrapBuild(){
  ...
  function tick(){wrapBuild();enhanceAll();}
