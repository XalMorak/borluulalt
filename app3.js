...
function runReport(){
  ...
  const locEl=document.getElementById("reportLoc");
  const loc=locEl?locEl.value:"";
  let rows=getSubs().slice();
  if(from)rows=rows.filter(s=>s.date>=from);
  if(to)rows=rows.filter(s=>s.date<=to);
  if(emp)rows=rows.filter(s=>s.employeeId===emp);
  if(loc)rows=rows.filter(s=>s.location===loc);
  _reportRows=rows;
  let totalSold=0,totalIncome=0,totalDiff=0;
  rows.forEach(s=>{
    (s.items||[]).forEach(it=>{totalSold+=it.sold||0;totalIncome+=it.income||0});
    totalDiff+=(s.diff!=null?s.diff:0);
  });
  const box=document.getElementById("reportSummary");
  if(box)box.innerHTML=`<div class="summary-item"><div class="label">Илгээлт</div><div class="value">${rows.length}</div></div><div class="summary-item"><div class="label">Борлуулсан</div><div class="value">${totalSold}</div></div><div class="summary-item"><div class="label">Орлого</div><div class="value">${totalIncome.toLocaleString()}₮</div></div><div class="summary-item"><div class="label">Зөрүү</div><div class="value">${(totalDiff>=0?"+":"")+totalDiff.toLocaleString()}</div></div>`;
  const tbody=document.getElementById("reportBody");
  if(!tbody)return;
  tbody.innerHTML=rows.map(s=>{
    const calc=s.calcTotal!=null?s.calcTotal:(s.items||[]).reduce((a,i)=>a+(i.income||0),0);
    const diff=s.diff!=null?s.diff:((s.cashAmount||0)+(s.cardTotal||0)-calc);
    const cls=Math.abs(diff)<0.01?"diff-ok":(diff>0?"diff-over":"diff-short");
    return `<tr><td>${s.date}</td><td>${s.employeeName}</td><td>${s.shift||"—"}</td><td>${s.location||"—"}</td><td>${calc.toLocaleString()}</td><td class="${cls}">${(diff>=0?"+":"")+diff.toLocaleString()}</td></tr>`;
...
function exportCSV(){
  const rows=_reportRows.length?_reportRows:getSubs();
